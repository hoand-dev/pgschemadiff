...
def test_fk_action_values() -> None:
    assert FKAction.NO_ACTION == "no_action"
    assert FKAction.RESTRICT == "restrict"
    assert FKAction.CASCADE == "cascade"
    assert FKAction.SET_NULL == "set_null"
    assert FKAction.SET_DEFAULT == "set_default"


@pytest.mark.unit
def test_fk_match_values() -> None:
    assert FKMatch.SIMPLE == "simple"
    assert FKMatch.PARTIAL == "partial"
    assert FKMatch.FULL == "full"


@pytest.mark.unit
def test_constraint_deferrability_values() -> None:
    assert ConstraintDeferrability.NOT_DEFERRABLE == "not_deferrable"
    assert (
        ConstraintDeferrability.DEFERRABLE_INITIALLY_IMMEDIATE == "deferrable_initially_immediate"
    )
    assert ConstraintDeferrability.DEFERRABLE_INITIALLY_DEFERRED == "deferrable_initially_deferred"
...
_CONSTRAINT_TA: TypeAdapter[Constraint] = TypeAdapter(Constraint)  # type: ignore[type-arg]
