...
| P2-DOM-01c | Column deltas (Add/Drop/AlterType/SetDefault/Nullability/RenameColumn) | review | P2-DOM-01a | src/pgschemadiff/domain/delta/column.py, tests/unit/domain/delta/test_column.py | 1 | high | **PR #6 (external session, branch clever-cray-6b2yqf) NOW `mergeable_state: dirty` (CONFLICTED) — main advanced past its base `f76cd26`→`27f63d7` after PR #7 merge; PR #6's doc edits collide. Needs REBASE onto main. Branch is externally owned → NOT force-pushed by this session (§5/§7.1). Escalated to human. Still blocks d/e/f.** |
| P2-DOM-01d | Index deltas (Create/Drop/Replace, method/predicate/include changes) | blocked | P2-DOM-01a, **PR#6-merge** | src/pgschemadiff/domain/delta/index.py, tests/unit/domain/delta/test_index.py | 0 | high | Shares `domain/delta/__init__.py` with PR #6 — start only after PR #6 merges (§6). |
| P2-DOM-01e | Constraint deltas incl. FK (Add/Drop PK/Unique/Check/FK/Exclusion) | blocked | P2-DOM-01a, **P2-DOM-01d** | src/pgschemadiff/domain/delta/constraint.py, tests/unit/domain/delta/test_constraint.py | 0 | high | Serialized after 01d (shared `__init__.py`). |
| P2-DOM-01f | Schema + extension deltas (Create/Drop schema, Create/Drop/Alter extension) **+ retype `DeltaSet.deltas` to the concrete `Delta` discriminated-union alias** (RF-A `TODO(P2-DOM-01f)` in delta/base.py — closes the lossy-round-trip gap) | blocked | P2-DOM-01a, **P2-DOM-01e** | src/pgschemadiff/domain/delta/schema.py, src/pgschemadiff/domain/delta/base.py, tests/unit/domain/delta/test_schema.py | 0 | high | Lands LAST; composes global `Delta` union. |
...
| P2-DIFF-08 | `topo_sort.py` — Kahn ordering + cycle detection → `CyclicDependencyError` | done | P2-DOM-01a | src/pgschemadiff/application/diff/topo_sort.py, tests/unit/application/diff/test_topo_sort.py | 1 | high | **MERGED to main (rebase, human, 2026-06-26) via PR #7 — main now `27f63d7`, CI green.** |
| TD-TOPO-01 | Tech-debt (reviewer N1–N5): refactor topo_sort `id()`-keyed O(n·m) → hashable-node dict O(n+m); test-hardening (assert non-cycle node absent from cycle msg; Hypothesis DAG property test); doc fix re removed `DeltaOp.NO_CHANGE` | review | P2-DIFF-08 | src/pgschemadiff/application/diff/topo_sort.py, tests/unit/application/diff/test_topo_sort.py, docs/PROJECT_CONTEXT.md, pyproject.toml | 1 | low | **PR #8 (`08879cb`): verify + code-reviewer(opus) BOTH GREEN/APPROVED — real `uv run mypy` clean (90 files), import-linter 4/4, pytest 720 passed, cov 87.2%; nitpicks RF-N1/N2 applied. → READY TO MERGE pending PR-CI green (human gate). Subscribed.** |
