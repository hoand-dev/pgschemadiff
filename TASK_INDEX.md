...
| P2-DIFF-08 | `topo_sort.py` — Kahn ordering + cycle detection → `CyclicDependencyError` | review | P2-DOM-01a | src/pgschemadiff/application/diff/topo_sort.py, tests/unit/application/diff/test_topo_sort.py | 1 | high | **PR #7 (branch clever-cray-9np03s, `f410486`): CI green 12/12 + reviewed (B1 mypy fixed by ci-recovery), 0 open review-fix → READY TO MERGE (human gate).** |
| TD-TOPO-01 | Tech-debt (reviewer N1–N5): refactor topo_sort `id()`-keyed O(n·m) → hashable-node dict O(n+m); test-hardening (assert non-cycle node absent from cycle msg; Hypothesis DAG property test); doc fix re removed `DeltaOp.NO_CHANGE` | ready | P2-DIFF-08 | src/pgschemadiff/application/diff/topo_sort.py, tests/unit/application/diff/test_topo_sort.py, docs/PROJECT_CONTEXT.md | 0 | low | Non-blocking; maintainer pass or fold into P2-DIFF-01. |
